/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/default-param-last, @typescript-eslint/prefer-for-of, @typescript-eslint/no-shadow */
 ...
 */
function combineTokens(tokenSources: any[]) {
...
function addMetadata(chainId: string, tokens: any[], tokenList: any) {
  ...
  return tokens.map((token: any) => {
    ...
    const tokenMetadata = checksumAddress && tokenList?.[checksumAddress];
    if (tokenMetadata) {
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    const tokensAddressesWithBalance = Object.entries(balances)
      ...
      .sort(([, balanceA], [, balanceB]) =>
        lte(balanceB as any, balanceA as any) ? -1 : 1,
      )
...
export const swapsTopAssetsSelector = createSelector(
  ...
  (chainId, tokens, tokenList, topAssets) => {
    ...
    const result = topAssets
      .map(({ address }: any) =>
        tokens?.find((token: any) => areAddressesEqual(token.address, address)),
      )
      .filter(Boolean);
    return addMetadata(chainId, result, tokenList);
  },
...
export interface SwapsState {
  ...
  featureFlags?: unknown;
  [chainId: string]: boolean | unknown | ChainSwapsState;
}
